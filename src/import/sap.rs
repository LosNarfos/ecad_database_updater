...
pub struct PartFromSAP {
    ...
    #[serde(default, rename = "Currency 2720")]
    pub currency_2720: String,
    ...
    #[serde(default, rename = "Currency 2100")]
    pub currency_2100: String,
...
fn clean_up_csv_file_from_sap() -> Result<(), std::io::Error> {
    ...
    println!("  Create new temp CSV file");
    let mut output_file = File::create("SAP_Export\\Extract_SAP4Zuken_fixed.csv")?;

    // then we open the bugged csv file given by the SAP-Output-Job
    println!("  Open bugged CSV file");
    ...
    println!("  Decode bytestream of bugged CSV file");
...
pub fn import() -> Result<Vec<PartFromSAP>, std::io::Error> {  
    ...
    for (index, row) in rdr.deserialize().enumerate().skip(2) {
        match row {
            Ok(row) => {
                parts.push(row);
            }
    ...
    // println!("  Reading fixed CSV file");
    // let path = "SAP_Export\\Extract_SAP4Zuken.csv";
    // let file = File::open(path)?;

    // // create reader from file and deserialize every row
    // let mut parts: Vec<PartFromSAP> = Vec::new();
